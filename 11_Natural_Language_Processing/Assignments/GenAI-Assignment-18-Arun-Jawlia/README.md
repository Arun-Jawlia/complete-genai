# Assignment 18: Text Cleaning, Preprocessing and NLP Pipeline

---
## Part 1 : NLP Pipeline & Basic Text Cleaning
- Task 1: Understanding Raw Text Data
- Task 2: Basic Text Cleaning

## Part 2 : Advanced Text Cleaning
- Task 3: Removing Noise
- Task 4: Handling Stopwords
- Task 5: Handling Repeated Characters and Slang (Optional)
  
## Part 3 : Basic Text Preprocessing in NLP
- Task 6: Tokenization
- Task 7: Stemming
- Task 8: Lemmatization
- Task 9: Final NLP Pipeline Creation
- Task 10: Observation & Insights


## How To run
1. UnZip the folder `GenAI-Assignment-18-Arun-Jawlia`
2. Open `Assignment-18.ipynb`
3. Run all